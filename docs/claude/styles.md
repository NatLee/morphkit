...
| `≤640px` (image editor) | `.editor-overlay > .editor:has(.ie-layout)` → **100dvh GRID, paint-app style**: areas 'head head'/'rail canvas'/'rail options'/'foot foot', 50px left column; `.ed-toolbar` = vertical scrolling tool RAIL, `.ed-options` thin bottom strip, `.kbd-hints` hidden; .ie-viewport flex + .ie-inner margin:auto centers the canvas |
...
| `≤640px` (gif/video modals) | `.editor:has(.gif-preview)` / `:has(.mp-video)` → 100dvh flex column: preview flexes to own the screen, transport + film strip / trim stay pinned, `.ed-controls` scrolls in place (26vh gif / 44vh video cap); audio keeps the plain sheet |
