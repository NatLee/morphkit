...
| `≤640px` OR `≤540px tall` (gif/video modals) | `.editor:has(.gif-preview)` / `:has(.mp-video)` → 100dvh flex column: preview flexes to own the screen, transport + film strip / trim stay pinned, `.ed-controls` scrolls in place (26vh gif / 44vh video cap); audio keeps the plain sheet |
