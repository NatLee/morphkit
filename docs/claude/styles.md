...
| app-mode (studio focus) | `.st-focus-btn` shows (hidden on desktop); `.studio.st-focus` hides `.st-assets`+`.split-gutter`, `body:has(.studio.st-focus)` hides `.topbar`/`.m-tabbar`/`.footer` — workspace owns the screen; `.studio.st-focus` gets a REAL `height` 100dvh−24 (not just min-height — WebKit doesn't stretch flex children from min-height alone; full-bleed image variant = 100dvh exactly + app padding-bottom 0) + padding-top `14px + env(safe-area-inset-top)` (the hidden topbar carried the notch padding). The bar hosts everything: `.st-fab` (assets toggle, 38px bar button + `.st-fab-n` badge) and `.st-save` (save proxy — shown via `:has(.ed-foot)`, dimmed via `:has(.btn-accent:disabled)`); the editors' `.ed-foot` is display:none, `.st-back-txt` + the bar spacer hidden, `.st-name` flexes. Bottom is clear → `.st-main` padding-bottom 10px+safe (0+safe full-bleed). **Image projects go FULL-BLEED**: `:has(... .ie-layout)` zeroes the app gutter + studio side/bottom padding, strips the editor card (border/radius/shadow/padding) and the viewport frame (padding 10), un-rounds the options strip; `.st-bar` keeps its own 12px gutter; other project types keep their cards |
